...
def plot_data(ys_data_dict, xlabel, ylabel, filename, x_data=None, ylim=None, linewidth=4):
    """
    Generic plotting function to handle repetitive plotting tasks.
    """
    plt.figure(figsize=(8, 6))
...
def compute_bounds_inf(perf_ref, perf_lrn, perf_bas=None):
    """
    Computes regret and confidence bounds.
    """
    res_lrn = {}
...
def process_and_plot_inf(prob_err, indx_err, perf_ref, perf_lrn, suffix, path, key_value, perf_bas=None, perf_stt=None):
    ...
    plot_data(arg_for_perf_stt, 'Episodes', 'Objective', f'{path}perf_{suffix}_{key_value}.pdf')
