...
def plot_reg_inf(y_data, xlabel, ylabel, filename, x_data=None, ylim=None, linewidth=4, fill_bounds=None):
    """
    Generic plotting function to handle repetitive plotting tasks.
    """
    plt.figure(figsize=(8, 6))
...
def plot_data_inf(ys_data_dict, xlabel, ylabel, filename, x_data=None, ylim=None, linewidth=4):
    """
    Generic plotting function to handle repetitive plotting tasks.
    """
    plt.figure(figsize=(8, 6))
    # Define a list of colors to use for different lines using a colormap
    colors = plt.cm.get_cmap('tab10', len(ys_data_dict.keys()))
...
def compute_bounds_inf(perf_ref, perf_lrn):
    """
    Computes regret and confidence bounds.
    """
    avg_creg = numpy.mean(numpy.sum(perf_ref - perf_lrn, axis=2), axis=0)
    std_creg = numpy.std(numpy.sum(perf_ref - perf_lrn, axis=2), axis=0)
    min_creg = numpy.min(numpy.sum(perf_ref - perf_lrn, axis=2), axis=0)
    max_creg = numpy.max(numpy.sum(perf_ref - perf_lrn, axis=2), axis=0)
    avg_reg = [avg_creg[k] / (k + 1) for k in range(len(avg_creg))]
    return avg_reg, avg_creg, (avg_creg - std_creg, avg_creg + std_creg), (avg_creg - 3*std_creg, avg_creg + 3*std_creg), (min_creg, max_creg)


def process_and_plot_inf(perf_ref, perf_lrn, suffix, path, key_value):
    """
    Processes data and generates all required plots for a given suffix.
    """
    reg, creg, bounds, boundstri, minmax = compute_bounds_inf(perf_ref, perf_lrn)

    plot_reg_inf(creg)
    plot_reg_inf(creg, 'Episodes', 'Cumulative Regret', f'{path}cumregbounds_{suffix}_{key_value}.pdf', fill_bounds=bounds)
    plot_reg_inf(creg, 'Episodes', 'Cumulative Regret', f'{path}cumregboundstri_{suffix}_{key_value}.pdf', fill_bounds=boundstri)
    plot_reg_inf(creg, 'Episodes', 'Cumulative Regret', f'{path}cumregminmax_{suffix}_{key_value}.pdf', fill_bounds=minmax)
    plot_reg_inf(reg, 'Episodes', 'Regret', f'{path}reg_{suffix}_{key_value}.pdf')
